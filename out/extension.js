...
const getAllLuaExtensionPaths = async (arrFiles) => {
...
const getLocalizedFuncs = (regionContent) => {
    const funcs = new Map();
    let match;
...
function activate(context) {
    const disposable = vscode.commands.registerCommand('lua-localizer.localize', async () => {
        ...
        const paths = vscode.workspace.getConfiguration("Lua").get("workspace.library") || [];
        if (paths.length === 0) {
        ...
        const localizedCalls = regionFound ? getLocalizedFuncs(regionFound[1]) : new Map();
        if (regionFound) {
        ...
        await editor.edit(editBuilder => {
            ...
            const regionContent = '--#region Localizations ---------------------------------------------------------------------\n\n' +
